...
| 类型 | 支持情况 |
| --- | --- |
| 操作系统 | macOS / Linux / Windows 理论可运行；打开本地文件夹接口按平台适配 |
| Python | `>=3.10,<3.13`，仓库当前 `.python-version` 为 `3.11` |
| 包管理 | uv |
| 后端 | FastAPI + Uvicorn |
| 前端 | 原生 HTML / CSS / JavaScript ES Modules |
| 存储 | SQLite，WAL 模式 |
| 视频处理 | yt-dlp、ffmpeg、ffprobe，硬烧录需要 ffmpeg 支持 libass |
...
| 硬烧录失败或缺少 `subtitles` filter | 安装支持 libass 的 ffmpeg，或临时使用 `--burn soft` |
